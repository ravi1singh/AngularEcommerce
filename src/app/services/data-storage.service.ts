...
export class DataStorageService {
  ...
  loginValidation(login:Login):Observable<UserDetails>{
      console.log("in dataservice");
       return this.http.post<UserDetails>("http://localhost:8080/login",login);
  ...
  //  }
   addToCart(addToCart:AddToCart,list:Product):Observable<boolean>{
    ...
    console.log(this.cartlist);
     return this.http.post<boolean>("http://localhost:8080/addtocart",addToCart);
   }
   checkout(checkout:Checkout):Observable<boolean>{
      var flag= this.http.post<boolean>("http://localhost:8080/checkout",checkout);
      if(flag){
        this.cartlist=[];
